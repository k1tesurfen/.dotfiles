...
return {
  ...
  config = function()
      ...
      vim.system(
        ...
        function(result)
          vim.schedule(function()
            ...
            if not ok then
              vim.notify("Failed to parse JSON from decouvertes CLI: " .. tostring(card), vim.log.levels.ERROR)
              stop_game()
              return
            end
    ...
    vim.keymap.set("n", "<leader>dv", start_game, { noremap = true, silent = true, desc = "decouvertes: Start review" })
